...
| `META_CONFLUENCE` | Meta | STRONG_BULLISH / BULLISH / STRONG_BEARISH / BEARISH / MIXED |
...
Konfiguracja: `ma_crossover_criteria.fast_period`, `slow_period`, `use_ema`.
...
Meta-analizator agregujący sygnały ze wszystkich pozostałych modułów dla danego tickera. Emituje sygnał zbieżności, gdy wiele niezależnych modułów wskazuje ten sam kierunek.

**Dane:** tabela `trade_signals` w SQLite (ostatnie N dni)

| Sygnał | Warunki |
|--------|--------|
| 🟢🟢 STRONG BULLISH | ≥ `min_signals` modułów byczych, 0 niedźwiedzich |
| 🟢 BULLISH | Więcej byczych niż niedźwiedzich (≥ 2 bycze) |
| 🔴🔴 STRONG BEARISH | ≥ `min_signals` modułów niedźwiedzich, 0 byczych |
| 🔴 BEARISH | Więcej niedźwiedzich niż byczych (≥ 2 niedźwiedzie) |
| 🟡 MIXED | Istotne sygnały w obu kierunkach |
...
Konfiguracja: `confluence_criteria.lookback_days` (domyślnie 3), `min_signals` (domyślnie 3).
