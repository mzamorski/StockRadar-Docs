...
Dla `REPORT_AI_DAILY_PICK` menu udostępnia dwa osobne rankingi skuteczności: **wg providera** (np. ChatGPT, Gemini, Claude, Grok) oraz **wg konkretnej wersji LLM** (np. GPT-5.6 Sol, Gemini 2.5 Pro, Claude Sonnet 4.6). Po wybraniu raportu użytkownik wskazuje jeden horyzont: 1, 7, 14 albo 30 dni, dzięki czemu tabela pokazuje tylko jeden ranking zamiast czterech sekcji naraz. Ranking obejmuje sygnały kierunkowe AI oraz `HOLD`; wpisy manualne są pomijane. W tym raporcie `HOLD` jest interpretowany jako **prognoza neutralnego zachowania kursu**, a nie jako zalecenie dalszego posiadania już otwartej pozycji. `BUY`/`SELL` są oceniane jak dotychczas po dodatnim wyniku kierunkowym netto po kosztach, natomiast `HOLD` nie otwiera pozycji i jest oceniany bez kosztu transakcyjnego: jest trafiony, gdy bezwzględna zmiana kursu mieści się w paśmie neutralności zależnym od horyzontu. Domyślna konfiguracja to `1D ±1%`, `7D ±2%`, `14D ±3%` i `30D ±5%`, ustawiana w `ai_pick_performance.hold_tolerance_pct` w `config.yaml`. Tabela pokazuje skuteczność łączną oraz osobno liczbę i skuteczność sygnałów kierunkowych i `HOLD`; średni wynik netto dotyczy wyłącznie sygnałów kierunkowych, aby neutralne rekomendacje nie zniekształcały zwrotu. Dla historycznych rekordów, w których zapisano tylko nazwę providera bez wersji modelu, raport wersji LLM pokazuje jawnie etykietę typu `ChatGPT (wersja nieznana)` zamiast zgadywać model. Presety menu zapisują CSV z horyzontem w nazwie, np. `ai_daily_pick_provider_performance_7d.csv` i `ai_daily_pick_model_performance_7d.csv`, w systemowym katalogu tymczasowym `StockRadar`.
...
- `--ai-pick-performance <provider|model>` - raport skuteczności `REPORT_AI_DAILY_PICK` wg rodziny providera albo konkretnej wersji LLM
- `--ai-pick-horizons <D1,D2,...>` - horyzonty raportu AI; menu przekazuje jeden z: `1`, `7`, `14`, `30`
- `--ai-pick-export <PATH.csv>` - opcjonalny eksport wybranego rankingu AI do CSV
...
- Nazwy modeli AI są normalizowane na wspólnej granicy zapisu `trade_signals`, niezależnie od tego, czy sygnał pochodzi z analizatora, CLI, Telegrama czy importu. Pola `source_name` i `ai_model` otrzymują tę samą kanoniczną nazwę, np. `gpt-5.6-sol` i `GPT-5.6 Sol` są zapisywane jako `GPT-5.6 Sol`; wybór `manual` nie jest klasyfikowany jako źródło AI.
